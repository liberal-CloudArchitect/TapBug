#!/usr/bin/env python3
"""Validate the isolated V3 prompt set and build/sign its nine role manifests."""
...
REGISTRY_PATH = Path("prompts/v3/registry.json")
...
class V3AssetError(ValueError):
    """The V3 registry, prompt set, or agent binding is not trustworthy."""
...
def _contained_file(root: Path, relative: object, required_parent: Path) -> Path:
    if not isinstance(relative, str) or not relative:
        raise V3AssetError("registry path must be a non-empty relative string")
    path = (root / relative).resolve()
    expected_parent = (root / required_parent).resolve()
    try:
        path.relative_to(expected_parent)
    except ValueError as exc:
        raise V3AssetError(f"registry path escapes {required_parent}: {relative!r}") from exc
    if not path.is_file():
        raise V3AssetError(f"registered asset is not a file: {relative!r}")
    return path
...
def _frontmatter(path: Path) -> dict[str, str]:
    ...
    if not lines or lines[0] != "---":
        raise V3AssetError(f"agent definition has no frontmatter: {path}")
    try:
        end = lines.index("---", 1)
    except ValueError as exc:
        raise V3AssetError(f"agent frontmatter is not terminated: {path}") from exc
    result: dict[str, str] = {}
    for line in lines[1:end]:
        key, separator, value = line.partition(":")
        if not separator or not key.strip() or not value.strip():
            raise V3AssetError(f"unsupported agent frontmatter line in {path}: {line!r}")
        key = key.strip()
        if key in result:
            raise V3AssetError(f"duplicate agent frontmatter key in {path}: {key!r}")
        result[key] = value.strip().strip("\"'")
    return result


def load_registry(root: Path = PROJECT_ROOT) -> tuple[dict[str, dict[str, Any]], str]:
    """Load and fully validate the V3 registry without consulting V2 runtime code."""

    registry_path = root / REGISTRY_PATH
    try:
        document = json.loads(registry_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise V3AssetError(f"could not load V3 prompt registry: {exc}") from exc
    if not isinstance(document, dict) or set(document) != {
        "version",
        "collection_sha256",
        "roles",
    }:
        raise V3AssetError("V3 prompt registry has unexpected top-level fields")
    roles = document.get("roles")
    if document.get("version") != "3" or not isinstance(roles, dict):
        raise V3AssetError("V3 prompt registry must be a version-3 role mapping")
    if set(roles) != EXPECTED_ROLES:
        raise V3AssetError("V3 prompt registry must contain exactly the nine Phase 4 roles")
    collection_digest = _canonical_digest(roles)
    if document.get("collection_sha256") != collection_digest:
        raise V3AssetError("V3 prompt registry collection digest mismatch")

    verified: dict[str, dict[str, Any]] = {}
    for role, raw in roles.items():
        if ROLE_PATTERN.fullmatch(role) is None or not isinstance(raw, dict):
            raise V3AssetError(f"invalid V3 role entry: {role!r}")
        if set(raw) != ENTRY_FIELDS:
            raise V3AssetError(f"V3 registry entry has unexpected fields for {role!r}")
        prompt = _contained_file(root, raw["prompt_path"], Path("prompts/v3"))
        agent = _contained_file(root, raw["agent_path"], Path("agents/v3"))
        digest = raw["prompt_sha256"]
        if not isinstance(digest, str) or DIGEST_PATTERN.fullmatch(digest) is None:
            raise V3AssetError(f"invalid V3 prompt digest for {role!r}")
        if _file_digest(prompt) != digest:
            raise V3AssetError(f"V3 prompt digest mismatch for {role!r}")
        prompt_version = raw["prompt_version"]
        if (
            not isinstance(prompt_version, str)
            or re.fullmatch(r"3\.[0-9]+", prompt_version) is None
        ):
            raise V3AssetError(f"invalid V3 prompt version for {role!r}")
        expected = {
            "name": role,
        ...
        if {key: _frontmatter(agent).get(key) for key in expected} != expected:
            raise V3AssetError(f"V3 agent frontmatter does not bind prompt for {role!r}")
        if raw["prompt_id"] != expected["prompt_id"]:
            raise V3AssetError(f"invalid V3 prompt ID for {role!r}")
        if raw["output_contract_id"] != OUTPUT_CONTRACT_IDS[role]:
            raise V3AssetError(f"invalid V3 output contract for {role!r}")
        if raw["operations"] != list(OPERATIONS[role]):
            raise V3AssetError(f"invalid V3 operation declaration for {role!r}")
        ipc = raw["allowed_ipc"]
        if (
            ...
            raise V3AssetError(f"invalid V3 IPC declaration for {role!r}")
...
def _immutable_image(value: str) -> str:
    ...
    if match is None or set(match.group(1)) == {"0"}:
        raise V3AssetError("image must use a non-placeholder immutable sha256 digest")
    return value
...
def _load_signer(private_key_path: Path):
    ...
    if not isinstance(key, Ed25519PrivateKey):
        raise V3AssetError("role manifest signer must be an Ed25519 private key")
    return key.sign
...
def generate_manifest_bundle(
    ...
    *,
    ...
    if re.fullmatch(r"[A-Za-z0-9._-]{1,128}", key_id) is None:
        raise V3AssetError("invalid manifest key ID")
    ...
    for role in sorted(registry):
        ...
        manifest: dict[str, Any] = {
            ...
            "input_schema": "task-envelope/v3",
            "output_schema": "handoff-envelope/v3",
            "limits": {
                "timeout_seconds": 180,
                "cpu_count": 1.0,
                "memory_mib": 256,
